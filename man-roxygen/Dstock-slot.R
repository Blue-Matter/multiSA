...
#' \item{`len_ymas`}{Length-at-age. Only needed if `Dmodel@nl > 0` to fit length composition (you may want to calculate the growth at the middle of the time step). [calc_growth()] may be a helpful function.}
...
#' \item{`mat_yas`}{Proportion mature by age class. Ignored if maturity ogive is estimated, e.g., when fitting to close-kin genetic data.}
#' \item{`swt_ymas`}{Stock weight-at-age (at beginning of time step). See [calc_growth()] example.}
#' \item{`fec_yas`}{Fecundity, i.e., spawning output, of mature animals. Default uses stock weight at age.}
#' \item{`M_yas`}{Natural mortality. Instantaneous units per year. Ignored if M is estimated.}
