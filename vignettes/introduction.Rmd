...
MSA (*Multi-stock Assessment with Regional Spatiotemporal dynamics*) is an age-structured model is intended to address these situations where an explicit multi-stock, seasonal, and spatial assessment model is desirable.
...
As a "next-generation" stock assessment package, MSA relies on state-of-the-art developments in computational power, optimization software, and along with important lessons learned from modeling mixed-stock marine fisheries, such as Atlantic bluefin tuna. 

MSA is entirely written in R and uses automatic differentiation enabled by RTMB for model fitting.

# Package overview

## Model fitting

Model fitting first starts with a S4 object of class `MSAdata`, with various slots to specify the model structure and data. See `help("MSAdata-class")`. `check_data()` scans the object and checks if the arrays are of the correct dimension and the necessary ancillary information are provided, e.g., sample sizes for compositional data.
...
These objects can then be passed to `fit_MSA()` which creates and fits the model:
...
View the options with `help("plot-MSA-data")` and `help("plot-MSA-state")`.
